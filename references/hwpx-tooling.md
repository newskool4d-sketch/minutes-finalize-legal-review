...
`apply_approved_edits.py`는 텍스트가 변경된 산출물의 `hp:linesegarray` 레이아웃 캐시를 결재용·정보공개용 모두에서 제거한다. 한글은 열 때 캐시를 다시 계산하므로, 변경 전 문장 길이를 기준으로 남은 캐시 때문에 열기 오류가 생기는 것을 막기 위한 조치다. 변경이력의 `layout_cache`에서 제거 건수를 확인한다.
